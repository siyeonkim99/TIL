...
??? note "post.model_dump() -> Postcreate 객체(pydantic)를 딕셔너리 형태로 변환"
    {"title": "제목입니다", "author": "홍길동", "content": "내용입니다"}  
    ** 은 키워드 인자로 풀어줌  
    Post(title="제목입니다", author="홍길동", content="내용입니다")  
    즉, DB 객체를 가져오는거임
...
??? note "사용자가 실제로 보낸(None이 아닌) 필드만 골라 수정하는 부분"
    1) model_dump()로 요청을 dict로 변환 → {"title": "새 제목", "content": None}  
    2) .items()로 (key, value) 쌍을 하나씩 꺼냄  
    3) if value is not None → 값 있는 것만 추림 → {"title": "새 제목"}  
    4) 아래 setattr(post, key, value)는 post.title = "새 제목"과 동일
